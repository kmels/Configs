...
promptConfig =
    XPC { font              = "-misc-fixed-*-*-*-*-20-*-*-*-*-*-*-*"
        ...
        , completionKey     = ( shiftMask, xK_grave )
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- TODO, ((modm  , xK_apostrophe), spawn "scrot -e 'mv $f ~/Desktop' && nemo ~/Desktop" ) 
...
--      , ((modm, xK_g), goToSelected defaultGSConfig)
    ...
    -- Move to next window ehh
    , ((modm              , xK_q     ), spawn "/home/user/.cabal/bin/xmonad --recompile; /home/user/.cabal/bin/xmonad --restart")
    ...
    [((m .|. modm, key), screenWorkspace sc >>= flip whenJust (windows . f))
        | (key, sc) <- zip [xK_backslash, xK_apostrophe] [0..]
